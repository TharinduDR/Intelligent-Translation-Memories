...
from algo.embeddings.infersent import get_embeddings, InferSent
...
result_file = configParser.get('infersent-config', 'RESULT_FILE')
...
model_version = 1
MODEL_PATH = "/data/infersent/encoder/infersent%s.pkl" % model_version
...
W2V_PATH = '/data/glove/glove.840B.300d.txt' if model_version == 1 else '/data/fasttext/crawl-300d-2M.vec'
model.set_w2v_path(W2V_PATH)

model.build_vocab(train["source"].astype(str).tolist() + test["source"].astype(str).tolist(), tokenize=True)
